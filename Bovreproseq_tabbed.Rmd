...
for (i in sampleid){
  ...
  results <- (paste(i,"_bovreproseq_results.tsv",sep=""))
  results_df <- read.csv(results,header = TRUE,sep = "\t")
  colnames (results_df) <- c("SAMPLEID","PATHOGEN","QC STATUS","READ COUNTS","ABUNDANCE(%)","RESULT","COMMENT") # column names
  ...
  table_html <- knitr::kable(results_df, align = "lllcccc", caption = "RESULTS SUMMARY - Based on abricate results with custom Bovreproseq database. For more info: Seemann T, Abricate, Github https://github.com/tseemann/abricate") %>%
    kable_styling()
  ...
  mapped_stat<-df[,c(1,3,4)] # sliced dataframe
  colnames (mapped_stat) <- c("AMPLICON","MAPPED READS","ABUNDANCE(%)") # column names
  print(knitr::kable(mapped_stat,align="lcc",caption = "NO. OF MAPPED READS. Generated using minimap2 (Li, H. (2018).Bioinformatics, 34:3094-3100. https://doi.org/10.1093/bioinformatics/bty191) and samtools (https://doi.org/10.1093/gigascience/giab008)") %>%
