...
for (i in sampleid){
  ...
  results <- (paste(i,"_bovreproseq_lims_report.tsv",sep=""))
  results_df <- read.csv(results,header = TRUE,sep = "\t")
  colnames (results_df) <- c("SAMPLEID","PATHOGEN","QC STATUS","READ COUNTS","RESULT") 
  ...
  table_html <- knitr::kable(results_df, align = "llccl", caption = "RESULTS SUMMARY - Based on abricate results with custom Bovreproseq database. For more info: Seemann T, Abricate, Github https://github.com/tseemann/abricate") %>%
    kable_styling()
